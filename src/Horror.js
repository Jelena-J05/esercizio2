...
function HorrorBooks() {
    ...
    while (randomBooks.length < 4) {
        const randomIndex = Math.floor(Math.random() * horrorBooks.length);
        if (!randomBooks.includes(horrorBooks[randomIndex])) {
            randomBooks.push(horrorBooks[randomIndex]);
        }
    }
    return (
        <Container className="px-0">
            <h3 className='mt-5 mb-3'>Horror</h3>
