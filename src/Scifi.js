...
function ScifiBooks() {
    ...
    while (randomBooks.length < 4) {
        const randomIndex = Math.floor(Math.random() * scifiBooks.length);
        if (!randomBooks.includes(scifiBooks[randomIndex])) {
            randomBooks.push(scifiBooks[randomIndex]);
        }
    }
    return (
        <Container className="px-0">
            <h3 className='mt-5 mb-3'>Scifi</h3>
